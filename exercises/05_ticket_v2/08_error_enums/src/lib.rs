use crate::TicketNewError::Title;
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(tick) => tick,
        Err(TicketNewError::Description) => return Ticket::new(title, "Description not provided".into(), status).unwrap(),
        Err(TicketNewError::Title { error_type }) => panic!("{}", error_type),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(Title { error_type: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(Title { error_type: "Title cannot be longer than 50 characters".to_string() });
        }
